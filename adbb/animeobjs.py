...
class AniDBObj(object):
    ...
    def _fetch_anidb_data(self, block):
        adbb.log.debug("Sending anidb request for {}".format(self))
        ...
        if self._illegal_object:
            ...
            adbb.log.debug("{} is not a valid AniDB object".format(self))
    ...
    def __getattr__(self, name):
        ...
        if local_name[1:] in local_vars:
            return local_vars[local_name[1:]]
...
class Anime(AniDBObj):
    def __init__(self, init):
        ...
        for x in self.titles:
            if x.lang is None and x.titletype == 'main':
                self._title = x.title
                break
            ...
            for r in relations:
                ...
                for sr in self.db_data.relations:
                    if r.related_aid == sr.related_aid:
                        ...
                        sr.anime_pk = self.db_data.id
                        new_relations.append(sr)
                if not found:
                    r.anime_pk = self.db_data.id
...
class AnimeTitle(AniDBObj):
    __bases__ = "AnimeTitle"
    _aid = None
    _titletype = None
    _lang = None
    _title = None

    def __init__(self, aid, titletype, lang, title):
        super(AnimeTitle, self).__init__()
        if aid:
            adbb.log.debug("aid: {}, titletype: {}, lang: {}, title: {},".format(aid, titletype, lang, title.encode('utf-8')))
            self._aid = aid
    ...
    def __repr__(self):
        return "AnimeTitle(aid='{}', titletype='{}', lang='{}', title='{}')".format(
            self._aid,
            self._titletype,
            self._lang,
    ...
    def _get_db_data(self, close=True):
        sess = self._get_db_session()
        res = sess.query(AnimeTitleTable).filter_by(aid=self.aid).filter_by(titletype=self.titletype).filter_by(lang=self.lang).all()
        if len(res) > 0:
            self.db_data = res[0]
            # adbb.log.debug("Found db_data for anime_title: {}".format(self.db_data))
        else:
            if self.aid:
                sess = self._get_db_session()
                adbb.log.debug("Adding anime_title to db_data: {}".format(self.title.encode('utf-8')))
                new = AnimeTitleTable()
                new.aid = self.aid
                new.titletype = self._titletype
                new.lang = self.lang
...
class AnimeDesc(AniDBObj):
    ...
    def __init__(self, aid, part=None, max_part=None, description=None):
        super(AnimeDesc, self).__init__()
        # self._desc_updated = threading.Event()
        if part:
            self._part = part
        if max_part:
        ...
        if aid:
            self._aid = aid
            adbb.log.debug("AnimeDesc for aid='{}', part='{}', max_part='{}', description='{}'".format(
                self._aid, self._part, self._max_part, self._description))
            if max_part is None:
                self.db_data = None
                self._get_db_data()
    ...
    def __repr__(self):
        if self._description:
            description = self._description.encode('utf-8')
        else:
            description = ''
        return "AnimeDesc(aid='{}', part='{}', max_part='{}', description='{}')".format(
            self._aid, self._part, self._max_part, description)

    def _get_db_data(self, close=True):
        sess = self._get_db_session()
        if self.part:
            res = sess.query(db.AnimeDescTable).filter_by(aid=self.aid).filter_by(part=self.part).all()
        else:
            res = sess.query(db.AnimeDescTable).filter_by(aid=self.aid).all()
        if len(res) > 0:
            self.db_data = res[0]
            self._aid = self.db_data.aid
            self._part = self.db_data.part
            self._max_part = self.db_data.max_part
            self._description = self.db_data.description
            adbb.log.debug("Found anime_description in db_data: {}".format(self.db_data))
        else:
            adbb.log.debug("Not found anime_description in db_data")
        if close:
            self._close_db_session(sess)

    def _anidb_file_data_callback(self, res):
        if res.rescode in ('330', '333'):
        ...
        if self.db_data:
            ...
            if new:
                ...
                self._aid = res.datalines[0]['aid']
                self._part = res.datalines[0]['part']
                self._max_part = res.datalines[0]['max_part']
                self._description = res.datalines[0]['description']
                self._updated.set()

    def _send_anidb_update_req(self, prio=False):
        adbb.log.debug("updating - aid: {} for part: {}".format(self._aid, self._part))
        if self._aid:
            self._updated.clear()
            adbb.log.debug("sending anime_description request with aid: '{}'".format(self._aid))
            req = AnimeDescCommand(aid=self._aid, part=self._part)
...
class File(AniDBObj):
    ...
    def __init__(
        ...
        adbb.log.debug("path: {}, fid: {}, anime: {}, episode: {}, lid: {}, ed2khash: {}".format(
            path.encode('utf-8'), fid, anime, episode, lid, ed2khash))
        if not path and not fid and not (anime and episode) and not lid and not ed2khash:
            raise AniDBError("File must be created with either filname, ed2khash, fid, lid or anime and episode.")
    ...
    def _send_anidb_update_req(self, prio=False, req_mylist=False, req_file=True):
        ...
        if req_mylist or not self._fid:
            if self._fid:
                ...
                if self._anime and self.episode:
                    req = MyListCommand(aid=self._anime.aid, epno=self.episode.episode_number)
                else:
                    adbb.log.debug("missing aid or epno to fetch mylist")
            if self._anime and self.episode:
                ...
                adbb.log.debug("aid or epno missing, not sending mylist request")
        self._updating.release()
    ...
    def _guess_anime_ep_from_file(self, aid=None):
        ...
        if not aid:
            ...
            if parent_dir:
                ...
                if series:
                    ...
                    adbb.log.debug("dir '{}': score {} for '{}'".format(
                        parent_dir.encode('utf-8'), series[0][2], series[0][3].encode('utf-8')))
                else:
            ...
            if not aid:
                ...
                if series:
                    adbb.log.debug(
                        "file '{}': trimmed to '{}', score {} for '{}'".format(filename.encode('utf-8'), joined, series[0][2],
                                                                               series[0][3]))
                    aid = series[0][0]
                else:
                    adbb.log.debug("file '{}': trimmed to '{}', no match".format(filename.encode('utf-8'), joined))
            if not aid:
                return (None, None)
