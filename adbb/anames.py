        ...
        if score > score_for_match or exact_match:
            titles = [
                    adbb.animeobjs.AnimeTitle(
                        aid=aid,
                        titletype=unicode(x.get('type')) if x.get('type') else None,
                        lang=unicode(get_lang_code(x.get('{http://www.w3.org/XML/1998/namespace}lang'))) if get_lang_code(x.get('{http://www.w3.org/XML/1998/namespace}lang')) else None,
                        title=unicode(x.text)) for x in anime.findall('title')]
            res.append((aid, titles, score, best_title_match))
